...
DIST_DIR=${INSIGHTS_DIST_DIR:-"${DEFAULT_DIST_DIR}"}
DEFAULT_REPOSITORY="https://github.com/RedHatInsights/rhosak-dashboard-build.git"
REPOSITORY=${INSIGHTS_REPOSITORY:-"${DEFAULT_REPOSITORY}"}
...
function usage() {
    ...
    echo "  --branch string             the branch where to push the dist (default: ${DEFAULT_BRANCH}) (env: INSIGHTS_BRANCH)"
    echo "  --dist-dir string           the dist directory to push (default: .${DEFAULT_DIST_DIR}) (env: INSIGHTS_DIST_DIR)"
    echo "  --repository string         the repo where to push the dist (default: \"${DEFAULT_REPOSITORY}\") (env: INSIGHTS_REPOSITORY)"
